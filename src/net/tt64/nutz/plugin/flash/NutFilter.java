...
import net.tt64.nutz.plugin.scoffold.ServerRedirectView;
...
public class NutFilter implements Filter {
	...
	public void init(FilterConfig conf) throws ServletException {
		...
		// @see Issue 301
		String skipMode = Strings.sNull(conf.getInitParameter("skip-mode"),
				"false").toLowerCase();
		if (!"true".equals(skipMode)) {
			handler = new ActionHandler(config);
			String regx = Strings.sNull(config.getInitParameter("ignore"),
					IGNORE);
	...
	public void doFilter(ServletRequest req, ServletResponse resp,
		...
		try {
			...
			if (needRealName && skipMode) {
				...
				Enumeration<String> names = Mvcs.getServletContext()
						.getAttributeNames();
				while (names.hasMoreElements()) {
					String name = (String) names.nextElement();
					if (name.endsWith("_localization")) {
						this.selfName = name.substring(0, name.length()
								- "_localization".length());
						break;
					}
				}
				needRealName = false;
			...
			if (!skipMode) {
				...
				if (null == ignorePtn
						|| !ignorePtn.matcher(path.getUrl()).find()) {
	...
	private void checkForRedirect(HttpServletRequest req) {
		...
		String oldReferer = (String) session
				.getAttribute(ServerRedirectView.REFERER);
