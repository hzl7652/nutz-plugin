package net.tt64.nutz.plugin.coc;
...
import net.tt64.nutz.plugin.scoffold.Scoffold;
...
public abstract class Loadings {
    ...
    public static void evalOk(ActionInfo ai, Ok ok, Method action ,Class<?> clazz) {
		if (null != ok) {
			...
			if(isController(clazz)){
				String pre = "jsp";
				try {
					if(Class.forName("net.tt64.nutz.plugin.scoffold.Scoffold") != null){
						if(null != action.getAnnotation(Scoffold.class)){
							pre = "vm";
						}
					}
				} catch (ClassNotFoundException e) {
					log.debug("nutz plugin scoffold is not found !!!");
				}
				String moduleName = getContrllerModule(clazz);
				String ctnName = getControllerName(clazz);
				if(Strings.isEmpty(moduleName)){
     ...
     * @param classZ
     * @return
     * @author feiyan35488, 凡是以Controller为结尾的均为模块类
     */
	...
	/**
	 * 
	 * 方法描述: 判断是否 为模块类， 不能为主模块，且以controller后缀
	 * @param clazz
	 * @return
	 */
